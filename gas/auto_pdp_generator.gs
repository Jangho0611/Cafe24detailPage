...
function buildDefaultNotes(data) {
  ...
  getCategoryDefaultNotes(data).forEach(function (note) {
    if (notes.length < 6 && notes.indexOf(note) === -1) {
      notes.push(note);
    }
  });
...

{
  ...
  "define": "[최대 2문장, 120자 이내 권장. 첫 문장은 제품명을 반복하지 말고 무엇으로 만든 제품인지와 어떤 구조인지 정의만 작성. 두 번째 문장은 대표 특징 또는 대표 사용처 중 하나만 자연스럽게 작성.]",
  "notes": [
...
- "~입니다", "~많이 사용됩니다" 중심으로 자연스럽게 작성한다.
- H2 바로 아래 첫 문장은 제품명을 반복하지 않는다.
- 첫 문장은 제품 정의만 작성한다. 무엇으로 만든 제품인지, 어떤 구조인지 설명한다.
- 두 번째 문장은 대표 사용 분야 또는 구조적 특징 중 하나만 자연스럽게 설명한다. 둘 다 억지로 작성하지 않는다.
- 최대 2문장, 120자 이내를 권장한다.
- 제품명 반복, AI 문체, 광고 문구, 과장 표현을 금지한다.
- "제품명은", "제품명는"처럼 제품명으로 시작하지 않는다.
- 최고의, 우수한, 프리미엄, 혁신적인, 적합합니다, 확인할 수 있습니다, 일반적으로 표현을 사용하지 않는다.
- 특히 define에는 적합합니다, 확인할 수 있습니다, 일반적으로, 우수한, 최고의, 프리미엄, 뛰어난, 뛰어나, 내구성이 뛰어나, 고급, 최적화, 최적화된 표현을 절대 사용하지 않는다.
- 대체 표현: 적합합니다 -> 많이 사용됩니다 / 확인할 수 있습니다 -> 주문 전 확인합니다 또는 자료 기준으로 표기합니다 / 일반적으로 -> 현장에서 또는 주로 / 우수한 -> 안정적인, 균일한, 일정한 / 내구성이 뛰어나 -> 반복 사용에 맞춰 검토됩니다 또는 구조 안정성을 고려해 사용됩니다 / 고급 가구 -> 가구 제작 또는 노출 마감 가구 / 최적화된 -> 많이 사용하는 또는 작업에 맞춰 선택하는.
- define 문체 예시: 균일한 단판을 여러 겹 적층한 합판입니다. 가구 제작과 CNC 가공, 노출 마감 작업에 많이 사용됩니다.
- define 예시1: 자작나무 단판을 여러 겹 적층한 합판입니다. 가구 제작과 노출 마감 작업에 많이 사용됩니다.
- define 예시2: 균일한 단판을 적층한 구조의 합판입니다. CNC 가공과 인테리어 제작에 주로 사용됩니다.
- define 예시3: 목재 단판을 적층하여 만든 판재입니다. 가구와 실내 마감 작업에 사용하는 제품입니다.
